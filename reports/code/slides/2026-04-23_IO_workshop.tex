...
\begin{itemize}
    ...
    \begin{itemize}
        \item These days, the operational restrictions embedded in PPAs is usually directly inherited from Performance Warranties.
    \end{itemize}
\end{itemize}
\end{frame}
...
\addlinespace
Route 66 / Sky Ranch II (NM-23-00353) & 50/100 & 365 & $<$40\% BAASOC ceiling  \\
\addlinespace
...
\begin{itemize}
    ...
    \item \textbf{Avg SoC ceiling} ($<$50\% or $<$40\% BAASOC)
    \item \textbf{Avg SoC floor} ($>$51\%)
    \item \textbf{Per-contract bundles}: actual combined restrictions from each scraped ESA
\end{itemize}

\vspace{0.3cm}
Under perfect foresight, yields \textit{upper-bound} estimate of profit loss attributable to each restriction (or restriction bundle).
\end{frame}
...
\vspace{0.1cm}
\footnotesize
Solve per regime (and per-contract bundles) on realized ERCOT prices; compare profit, cycles/day, and hour-of-day dispatch.
\end{frame}
...
\begin{frame}{Future steps / requested feedback}
    \begin{itemize}
        ...
        \end{itemize}
        \item Per-project profit-loss rankings across the 16 scraped contracts
        \item Modeling ``at-rest'' SoC constraints (MILP extension)
        \item Comparing simulated optimal dispatch to observed ERCOT dispatch / bids
    \end{itemize}
\end{frame}
