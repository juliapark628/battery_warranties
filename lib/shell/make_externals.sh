...
if [ "$create_links" = true ]; then
    mkdir -p "${REPO_ROOT}/data_cleaning/external"
    mkdir -p "${REPO_ROOT}/exploratory_figures/external"
    mkdir -p "${REPO_ROOT}/regression_scripts/external"
fi
...
while [ "$i" -lt "$((array_start_index + array_length))" ]; do
    ...
    if [ -n "$name" ]; then
        if [ -d "$target_path" ]; then
            ln -sfn "$target_path" "${REPO_ROOT}/data_cleaning/external/$name"
            ln -sfn "$target_path" "${REPO_ROOT}/exploratory_figures/external/$name"
            ln -sfn "$target_path" "${REPO_ROOT}/regression_scripts/external/$name"
