...
\title{Grid-Scale Battery Dispatch Distortions \\ Due to Operational Restrictions}
\author{Julia Park}
\date{April 20, 2026}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item These days, the operational restrictions from these are usually harmonized 
    \end{itemize}
\end{itemize}
\end{frame}
...
The industry has been interested in how to design these operational restrictions such that they are  \textbf{less restrictive}  (e.g. limits on number of cycles) and  \textbf{more flexible} (e.g. rolling averages, charge/discharge rates over the day).
...
\begin{itemize}
    \item \textbf{Electricity storage arbitrage}: Optimal battery dispatch, effects on market prices, frictions preventing storage from realizing theoretical value \textcolor{gray}{\small Butters, Dorsey, and Gowrisankaran (2025); Lamp and Samano (2022); Reynolds (2025); Denholm and Margolis (2018)}
    \item \textbf{Battery warranty design}: \textcolor{gray}{\small  Liu and Wang (2025)}
    \item \textbf{Battery degradation}: Engineering foundation relating operational behavior (DoD, C-rate, SOC, cycles) to capacity loss \textcolor{gray}{\small Xu et al.\ (2018); Attia et al.\ (2022)}
\end{itemize}
\textbf{Contribution}: 
\begin{itemize}
    \item Document specific operational restrictions embedded in grid-scale battery contracts
    \item Simulate dispatch behavior under different contract designs
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item $s_h \in [0, k]$: state of charge, with upper bound capacity $k$ (MWh)
